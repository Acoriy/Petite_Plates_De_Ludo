      ...
      <motion.header variants={containerVariants} initial="hidden" animate="show" className="mb-6">
        <div className="flex items-center justify-between gap-4">
          <motion.div variants={childFadeUp} className="space-y-1">
            <h1 className="text-3xl sm:text-4xl md:text-5xl font-display font-semibold leading-tight">{recipe.title}</h1>
        ...
        <motion.div variants={childFadeUp} className="mt-4 flex flex-wrap gap-4 text-sm text-muted-foreground items-center">
          <span className="inline-flex items-center gap-2"><Clock className="h-4 w-4" /> {recipe.prep_time + recipe.cook_time} min</span>
          <span className="inline-flex items-center gap-2"><Users className="h-4 w-4" /> {recipe.servings} pers.</span>
          <span className="inline-flex items-center gap-2"><Heart className="h-4 w-4" /> {recipe.likes} likes</span>
        </motion.div>
      </motion.header>
        ...
        <aside className="md:col-span-4 lg:col-span-3">
          <div className="sticky top-20 space-y-4">
            ...
            <motion.div variants={childFadeUp} className="rounded-2xl border p-6 shadow bg-white">
              ...
              <p className="text-sm text-muted-foreground mt-1">Mode cuisine — touchez pour cocher</p>
                  ...
                  <li key={i} className="relative">
                    ...
                    >
                      <motion.span
                        layout
                        initial={false}
                        animate={checked[i] ? { backgroundColor: '#0ea5a4', borderColor: '#0ea5a4' } : { backgroundColor: 'transparent', borderColor: '#d1d5db' }}
                        className={`inline-flex h-7 w-7 items-center justify-center rounded-md border flex-shrink-0`}
                      >
                        <AnimatePresence>
                          {checked[i] ? <motion.span initial={{ scale: 0.6, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.6, opacity: 0 }}><Check className="h-4 w-4 text-white" /></motion.span> : null}
                        </AnimatePresence>
                      </motion.span>
            ...
            <div className="no-print">
              <button onClick={printRecipe} className="w-full mt-2 inline-flex items-center justify-center gap-2 rounded-md border px-4 py-2 text-sm font-medium hover:bg-slate-50">
        ...
        <main className="md:col-span-8 lg:col-span-9">
          <motion.div variants={containerVariants} initial="hidden" animate="show">
            <section className="prose max-w-none">
              <motion.h2 variants={childFadeUp} className="mt-2 text-2xl font-display font-semibold">Préparation</motion.h2>
                ...
                {steps.map((s, idx) => (
                  ...
                  >
                    <div className="flex items-start gap-4">
                      ...
                      <div>
                        <div className="text-base font-medium leading-snug">{(s as any).text ?? s}</div>
                        {(s as any).image ? <img src={(s as any).image} alt={`Etape ${idx + 1}`} className="mt-3 max-h-72 w-full object-cover rounded-md shadow-sm" /> : null}
                      </div>
                    </div>
                  </motion.li>
                ))}
                ...
                {recommendations.map((r) => (
                  <article key={r.id} className="rounded-lg border overflow-hidden bg-white">
                    <a href={`/recette/${r.id}`} className="block">
                      {r.cover_image ? <img src={r.cover_image} alt={r.title} className="h-40 w-full object-cover" /> : <div className="h-40 w-full bg-slate-100" />}
                      <div className="p-3">
                        <h4 className="text-sm font-semibold">{r.title}</h4>
                        <p className="text-xs text-muted-foreground mt-1">{r.category?.name ?? ''}</p>
                      </div>
                    </a>
                  </article>
                ))}
